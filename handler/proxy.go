...
func (ps *ProxyServer) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	// log.Println(config.RuntimeViper.GetStringSlice("server.proxy_pass"))
	if req.URL.Path == "/getstatus" {
		var finalResponse []models.GStates
		var contentType string
		for _, el := range config.RuntimeViper.GetStringSlice("server.proxy_pass") {
			respUnmarshaled := models.GStates{}
			resp, err := http.Get("http://" + el + "/" + req.URL.RequestURI())
			if err != nil {
				rw.WriteHeader(http.StatusInternalServerError)
				log.Println(err)
				return
			}
			err = utils.DecodeMessageRes(resp, &respUnmarshaled)
			if err != nil {
				rw.WriteHeader(http.StatusInternalServerError)
				log.Println(err)
				return
			}
			finalResponse = append(finalResponse, respUnmarshaled)
			contentType = resp.Header.Get("Content-type")
		}
		log.Println(contentType)
		finalMarshaled, err := utils.EncodeResponse(finalResponse, strings.Split(contentType, "/")[1])
		if err != nil {
			rw.WriteHeader(http.StatusInternalServerError)
			log.Println(err)
			return
		}
		rw.Write(finalMarshaled)
		return
	}
	...
	} else if (req.Method == "GET" || req.Header.Get("Query-Type") == "select") && config.RuntimeViper.GetBool("server.cache") {
